...
for ax_nd_id, ax in np.ndenumerate(axes):
	...
	est_err_mom, est_err = gnuplot_utils.read_2d_curve(path + "addaptive_cc_eps_m" + str(eps_pow[ax_id]) + ".log", 4)
	err_mom,     err     = gnuplot_utils.read_2d_curve(path + "addaptive_cc_eps_m" + str(eps_pow[ax_id]) + ".log", 6)
...
plt.savefig('ClensawCurtisErrorEstimate.pdf', bbox_inches='tight')
plt.clf()
plt.cla()
